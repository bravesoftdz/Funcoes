...
function EscreveValorExtenso(valor: real): string;
function EscreveDataExtenso(Data:TDateTime): String;
...
function GetVersionApp(sFileName:string): string;
...
{ Convertendo números para sua representação em extenso,
 com a opção de valor monetário.
//
 É necessário estar declarado StrUtils e SysUtils na
 seção uses. }
//
function GetExtenso(Valor : Extended; Moeda: Boolean = False;
...
//
  if (Milhao <> '') then
    Milhao := Milhao + IfThen( Copy( Texto, 4,
      3 ) = '001', ' milhão', ' milhões');
//
  Bilhao := MiniExtenso( Copy( Texto,  1, 3 ) );
//
  if (Bilhao <> '') then
    Bilhao := Bilhao + IfThen( Copy( Texto, 1, 3 ) = '001',
    ' bilhão', ' bilhões');
//
  Result := Bilhao + IfThen( (Bilhao <> '') and
    (Milhao + Milhar + Centena <> ''), IfThen((Pos(' e ', Bilhao) > 0) or
    (Pos( ' e ', Milhao + Milhar + Centena ) > 0), ', ', ' e '), '') +
    Milhao + IfThen( (Milhao <> '') and (Milhar + Centena <> ''),
    IfThen((Pos(' e ', Milhao) > 0) or (Pos( ' e ', Milhar + Centena ) > 0 ),
    ', ',    ' e '), '') + Milhar + IfThen( (Milhar <> '') and
    (Centena <> ''), IfThen(Pos( ' e ', Centena ) > 0, ', ', ' e '),'')
    + Centena;
//
  if (Moeda) then
  begin
    ...
    begin
      if (Bilhao <> '') and (Milhao + Milhar + Centena = '') then
        ...
        if (Milhao <> '') and (Milhar + Centena = '') then
          ...
          IfThen((Pos(' e ', Bilhao) > 0) or
          (Pos( ' e ', Milhao + Milhar + Centena ) > 0),
          ', ', ' e '), '') + Milhao + IfThen( (Milhao <> '') and
          (Milhar + Centena <> ''), IfThen((Pos(' e ', Milhao) > 0)
          or (Pos( ' e ', Milhar + Centena ) > 0 ),', ',    ' e '), '')
          + Milhar + IfThen( (Milhar <> '') and (Centena <> ''),
          IfThen(Pos( ' e ', Centena ) > 0, ', ', ' e '),'') +
          Centena + IfThen( Int(Valor) = 1, ' real', ' reais');
//
      if (Centavos <> '') then
        if Valor > 1 then
          Result := Result + ' e ' + Centavos +
          IfThen( Copy( Texto, 14, 2 )= '01', ' centavo', ' centavos' )
        else
          Result := Centavos + IfThen( Copy( Texto, 14, 2 )= '01',
          ' centavo', ' centavos' );
    end
    else
    begin
      ...
      if (Milhao <> '') and (Milhar + Centena = '') then
        ...
        Result := Bilhao + IfThen( (Bilhao <> '') and
        (Milhao + Milhar + Centena <> ''), IfThen((Pos(' e ', Bilhao) > 0)
        or (Pos( ' e ', Milhao + Milhar + Centena ) > 0), ', ', ' e '), '')
        + Milhao + IfThen( (Milhao <> '') and (Milhar + Centena <> ''),
        IfThen((Pos(' e ', Milhao) > 0) or
        (Pos( ' e ', Milhar + Centena ) > 0 ),', ',    ' e '), '') +
        Milhar + IfThen( (Milhar <> '') and (Centena <> ''),
        IfThen(Pos( ' e ', Centena ) > 0, ', ', ' e '),'') + Centena +
        IfThen( Int(Valor) = 1, ' dolar americano', ' dolares americanos');
      if (Centavos <> '') then
        if (Valor > 1) then
          Result := Result + ' e ' + Centavos +
          IfThen( Copy( Texto, 14, 2 )= '01', ' cent', ' cents' )
        else
          Result := Centavos +
          IfThen( Copy( Texto, 14, 2 )= '01', ' cent', ' cents' );
    end;
